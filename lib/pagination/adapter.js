...
module.exports = class Adapter {
  constructor(content, page, limit, req) {
    ...
    this.limit = limit;
    this.request = req;
    if (this.page <= 0) throw invalidPageNumber('Invalid Page Number');
    if (this.limit <= 0) throw invalidLimitNumber('Invalid Limit Number');
  }
  ...
  pageUrl(page) {
    return page
      ? `${this.request.protocol}://${this.request.get('host')}${this.request.originalUrl}?page=${page}`
      : null;
  }
};
